...
class CLASS_CONV:
  ...
  def forward(self, input):
    # Padding applied
    self.input = np.pad(input, ((4, 4), (1, 1)), 'constant', constant_values=0) 
    print("Padded matrix: \n", self.input)
              ...
              for x in range(input_width):
                  ...
                  if x % self.strides == 0:
                      try:
                          ...
                          output_matrix[i][output_y, output_x] = (self.kernel[i] * self.input[y: y + kernel_height, x: x + kernel_width]).sum()
                      except:
                          break
