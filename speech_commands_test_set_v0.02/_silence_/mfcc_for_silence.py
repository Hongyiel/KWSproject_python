...
def merge_matrix(wav_matrix):
    ...
    x = np.array([0.0], dtype='f')
    new_matrix.extend(x)    
    return new_matrix

# get wav file from system OS
files = [f for f in os.listdir('/Users/hongyielsuh/Documents/GitHub/KWSproject_python/speech_commands_test_set_v0.02/_silence_') if os.path.isfile(f)]
...
np.savetxt('wav_data_silence_.csv', final_matrix, delimiter=',') 
